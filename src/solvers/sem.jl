...
通过 while 循环控制外层伪时间步, 直到 t_n 达到 t_max, 并记录 steps。
...
function solve_sem(
    ...
    α_init::Float64 = 2.0,           # AL 罚参数初始值
    β::Float64 = 1.2,                # 罚参数放大因子
    tol_staggered::Float64 = 1e-6,   # 交错收敛容差
    tol_kkt::Float64 = 1e-8,          # 内循环 KKT 容差
    max_staggered_iter::Int = 200,    # 最大交错迭代次数
    max_kkt_iter::Int = 200,           # 最大 KKT 迭代次数
    ...
    while t_n <= t_max # for step in 1:n_steps # 
        # --- 6a. BC ---
        # 归一化伪时间用于 BC 缩放 (BC 期望 [0,1] 范围)
        normalized_t = t_n / t_max
        ...
        println("=== 步 $step | t = $(round(t_n, digits=6)) | Δt = $(round(Δt, digits=6)) ===")
        ...
        for iter in 1:max_staggered_iter
            ...
            # ========================================
            k = 1
            λ_k = 0.0        # 重置 Lagrange 乘子
            α_k = α_init     # 重置罚参数
            d_n_k = copy(d_n) # AL 内循环的 d 迭代变量

            for kkt_iter in 1:max_kkt_iter
                # ========================================
                # 内层: Newton-Raphson 最小化求解 d_n_k (λ, α 固定)
                # ========================================
                for newton_iter_k in 1:max_newton_iter_d
                    # 计算当前 d_k 下的约束量
                    g = compute_g(d_n_k, d_prev, M_d, ρ)
                    diff_d .= d_n_k .- d_prev
                    mul!(M_diff, M_d, diff_d)

                    # 有效乘子 (使用外层的 λ, α, 不更新它们)
                    λ_eff = max(0.0, λ_k + α_k * g)
                    ...
                    # --- 更新 d ---
                    d_n_k .+= Δd
                    d_n_k .= clamp.(max.(d_n_k, d_prev), 0.0, 1.0)
                    apply!(d_n_k, ch_d)

                    # Newton 收敛检查
                    if norm(Δd) < tol_newton_d
                        break
                    end
                end # Newton 内循环
                ...
                viol = max(g, 0.0)
                comp = abs(λ_k * g)
                if viol <= tol_kkt && comp <= tol_kkt
                    d_n .= d_n_k # KKT 满足，更新 d_n 
                    break
                end
        ...
        if step % output_freq == 0
            VTKGridFile("data/sims/sem/fracture_sem_step_$step", dh_u) do vtk
    ...
    println("S-EM 仿真结束！VTK 文件保存在 data/sims/sem 目录下。")
    println("总载荷步数: $step")
